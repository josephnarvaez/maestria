...
public function insertar($usu_nombre,$usu_cedula,$usu_telefono,$usu_correo,$usu_cargo,$usu_login,$usu_clave,$permisos){
	$sql="call activos_ist17j.sp_insertausuario(0,'$usu_nombre','$usu_cedula','$usu_telefono','$usu_correo','$usu_cargo','$usu_login','$usu_clave','1')";
//	return $sql;
	...
	if ($row==false) {
	 ...
	 while ($num_elementos < count($permisos)) {
	 	$sql_detalle="CALL sp_insertapermisos(0,$idusuarionew[0],$permisos[$num_elementos])";
...
public function editar($usu_id,$usu_nombre,$usu_cedula,$usu_telefono,$usu_correo,$usu_cargo,$usu_login,$usu_clave,$permisos){
	$sql="call activos_ist17j.sp_insertausuario($usu_id,'$usu_nombre','$usu_cedula','$usu_telefono','$usu_correo','$usu_cargo','$usu_login','$usu_clave','1')";
	$row=ejecutarConsultaSP($sql);
	 $num_elementos=0;
	 $sw=true;
	 $sql_detalle="CALL horarios_ist17j.sp_insertapermisos(0,$usu_id,$permisos[$num_elementos])";
     //borro permisos 
	 $g=ejecutarConsulta($sql_detalle);
	 while ($num_elementos < count($permisos)) {
        //echo $permisos[$num_elementos];
	 	$sql_detalle="CALL horarios_ist17j.sp_insertapermisos(1,$usu_id,$permisos[$num_elementos])";
...
public function mostrar($usu_id){
	$sql="call activos_ist17j.sp_usuarios(1,$usu_id)";
	$row=ejecutarConsultaSP($sql);
...
public function listar(){
	$sql="call activos_ist17j.sp_usuarios(0,0)";
	return ejecutarConsultaSP($sql);
}
...
public function regevento($op,$cedula,$nombres,$apellidos,$cargo,$telefono,$correo,$institucion,$ciudad,$genero,$etnia){
	$sql="CALL regevento($op,'$cedula','$nombres','$apellidos','$cargo','$telefono','$correo','$institucion','$ciudad','$genero','$etnia');";
	return ejecutarConsultaSP($sql);
}
